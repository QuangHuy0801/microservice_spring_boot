...
public class UnitOfProductStatisticActivity extends AppCompatActivity {
    ...
    private void setupPieChart(){
        AdminAPI.adminApi.UnitOfProductStatistic().enqueue(new Callback<List<ReportTotal>>() {
            @Override
            public void onResponse(Call<List<ReportTotal>> call, Response<List<ReportTotal>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (reportTotals.isEmpty()) {
                        Toast.makeText(UnitOfProductStatisticActivity.this, "Không có dữ liệu", Toast.LENGTH_LONG).show();
                    } else {
                        ...
                        pie.title("Thống kê đơn vị sản phẩm: "+totalProducts+" đơn vị");
                    ...
                    Toast.makeText(UnitOfProductStatisticActivity.this, "Không thể lấy dữ liệu", Toast.LENGTH_LONG).show();
                }
            }

            @Override
            public void onFailure(Call<List<ReportTotal>> call, Throwable t) {
                Toast.makeText(UnitOfProductStatisticActivity.this, "Lỗi: " + t.getMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    private void saveExcel(HSSFWorkbook hssfWorkbook){
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
            fileOutput = new File(storageVolume.getDirectory().getPath() +"/Download/ThongKeDonViSanPhamKho.xls");
        }
        ...
        try {
            ...
            hssfWorkbook.close();
            Toast.makeText(this, "File Created Successfully: Download/ThongKeDonViSanPhamKho.xls", Toast.LENGTH_LONG).show();
